...
// browser. ESPN gives the *running* score plus a real match status and clock
// ("67'", "HT").
//
// Roles (see App.jsx merge order):
//   • The COMMITTED SCHEDULE (src/data/matches.js) = SOURCE OF RECORD. This
//     edition is finished, so every final score and scorer is already in the
//     repo, generated and triple-cross-checked at build time against FIFA by
//     scripts/fetch-tournament.mjs.
//   • ESPN (here) = LIVE OVERLAY, and the only runtime source. It matters for a
//     future edition, where it fills the gap while a match is underway.
//
// THIS APP HAS ONE RUNTIME SOURCE, NOT TWO. The sibling viewers reconcile ESPN
// against OpenFootball, which publishes the MEN'S World Cups only — there is no
// women's repo in any format. services/reconcile.js is source-agnostic and took
// the drop with no code change; with one source it confirms rather than
// cross-checks, so this app carries no confirmation badge and no final-score
// getter at all.
...
//
// Empty for the Women's World Cup 2023: ESPN's scoreboard names all thirty-two
// sides exactly as we do — verified against every one of the 64 matches and
// pinned by test/fixtures/espn-team-names.json.
//
// It is deliberately empty rather than inherited. The Euro/World Cup version of
// this table maps "United States" to "USA", which is right for those apps and
// actively WRONG here: "United States" IS our canonical name, so that entry
// rewrote it to a non-team and silently dropped every USA match from the live
// overlay. An unmapped-or-mis-mapped spelling fails quietly, which is why the
// test below refuses any alias key that never appears in the captured feed.
...
// dates=20260728 returns instants up to 07-29T02:00Z). An Oceania lunchtime kickoff
// lands just past midnight UTC — still "yesterday" in New York — so filing by the
// UTC day asked for a slate that didn't contain the match and the backfill missed
// it. The live window survives this only because it spans ±1 day.
...
// Overlay ESPN live / just-finished data onto the committed matches array. The
// committed schedule stays the source of record: if a match already has a score,
// it is left untouched. (In the sibling viewers that score comes from
// OpenFootball; here it was baked in at generation time.) The static input is
// never mutated.
