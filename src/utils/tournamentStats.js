...
// Group scorer spellings that differ only by diacritics ("Julián" vs "Julian"),
// which would otherwise split one player's tally in two. The sibling viewers
// need this because ESPN and OpenFootball disagree on accents; this edition has
// no OpenFootball feed, but the name join still crosses ESPN's scoreboard event
// feed and its separate leaders endpoint, so the key is kept as the defensive
// join for anything matching players by name (Boot extras, per-match lines).
