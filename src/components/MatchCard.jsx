...
// context (e.g. knockout placeholders). Only two routes exist per group — 3rd
// and 4th are out — so there is no conditional best-third line.
function slotTooltip(group, slot, clinch) {
  if (!group || !slot) return null
  const qf = (num) => `Round of 16 · Match ${num}`
  if (clinch === 'won-group') return `Clinched Group ${group} winner → ${qf(slot.win)}`
  if (clinch === 'runner-up') return `Clinched Group ${group} runner-up → ${qf(slot.runnerUp)}`
  if (clinch === 'eliminated') return `Eliminated from Group ${group} — no knockout slot`
  const parts = []
  if (slot.win) parts.push(`1st → ${qf(slot.win)}`)
  if (slot.runnerUp) parts.push(`2nd → ${qf(slot.runnerUp)}`)
  parts.push('3rd or 4th → eliminated')
