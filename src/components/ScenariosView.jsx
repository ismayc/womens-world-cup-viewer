              ...
              <td className="col-team">
                ...
                {tie && (
                  <span
                    ...
                    title={`Level with ${tie.vs} on points, goal difference, goals and head-to-head — separated by ${TIEBREAK_LABEL[tie.reason]}${tie.reason === 'conduct' ? ' (best-effort card data)' : ''}`}
...
// "As it stands → round of 16" line for a projected qualifier. `confirmed` is
...
export default function ScenariosView({ matches }) {
  ...
  // `lockedOpponent` only resolves from a settled group winner / runner-up, and a
  // round-of-16 tie locks only when BOTH of its groups are settled — so check both
  // sides of the tie rather than assuming the one we happen to be rendering is
  // the settled one.
  const lockedBetween = (a, b, matchNum) => {
    const l = lockedOpponent(synthetic, a, clinch)
    ...
    <div className="scenarios">
      <div className="sc-intro">
        <p>
          ...
          round of 16 would shake out. Deterministic — no predictions, just the consequences of
      ...
      <div className="sc-grid">
        {groupsInPlay.map((g) => {
          ...
          // Placings decided by a soft tie-breaker (cards / a drawing of lots).
          ...
          return (
            <div className="sc-card" key={g}>
              ...
              <div className="sc-r32">
                <div className="sc-r32-title">Projected round of 16</div>
      ...
      <p className="sc-foot">
        ...
        distinct final standings still reachable for a group given the results you’ve set. A
        round-of-16 tie pairs two different groups, so an exact opponent only firms up as you set
        results in both. A <ScalesIcon /> marks a placing decided by a soft tie-breaker — the
        disciplinary record or a drawing of lots — once points, goal difference, goals and
        head-to-head are all level (hover it for the decider; card data is best-effort).
      </p>
    </div>
  )
}
