...
const base = MATCHES.find((m) => m.num === 1) // New Zealand v Norway (Group A)
const knockout = MATCHES.find((m) => m.stage === 'QF')
...
describe('MatchCard one-off status', () => {
  ...
  it('abandoned: labeled partial score, no source confirmation, no FT', () => {
    renderCard({ ...base, voided: true, statusLabel: 'Abandoned', score: [1, 1] })
...
describe('MatchCard normal states (full coverage)', () => {
  ...
  it('finished: score, AET and the FT badge', () => {
    // No cross-source confirmation badge here: this app has ONE runtime source,
    // so the "confirmed by N sources" apparatus was removed as permanently inert.
    renderCard({ ...base, score: [2, 1], aet: true })
  ...
  it('spoiler reveal flow', () => {
    renderCard({ ...base, score: [3, 0] }, { hidden: true })
    expect(screen.getByText('tap to reveal')).toBeInTheDocument()
...
describe('MatchCard follow / clinch / slot tooltip', () => {
  it('toggles follow', () => {
    renderCard()
    fireEvent.click(screen.getByRole('button', { name: 'Follow New Zealand' }))
    expect(screen.getByRole('button', { name: 'Unfollow New Zealand' })).toBeInTheDocument()
  })

  it('won-group clinch badge + slot tooltip', () => {
    renderCard(base, { clinch: { 'New Zealand': 'won-group' } })
    expect(screen.getByText(/Won group/)).toBeInTheDocument()
    expect(screen.getByText('New Zealand').getAttribute('title')).toMatch(/Clinched Group A winner/)
  })

  it('eliminated slot tooltip', () => {
    renderCard(base, { clinch: { 'New Zealand': 'eliminated' } })
    expect(screen.getByText('New Zealand').getAttribute('title')).toMatch(/Eliminated from Group A/)
  })
