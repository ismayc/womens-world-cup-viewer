...
describe('NextMatch', () => {
  ...
  it('shows the tournament-concluded message when there is no match', () => {
    vi.setSystemTime(new Date('2023-08-21T00:00:00Z'))
  ...
  it('crowns the champion (and runners-up) once the Final is decided', () => {
    ...
    const final = {
      ...
      t1: 'Spain',
      t2: 'England',
      venue: 'stadiumaustralia',
      ko: '2023-08-20T20:00:00+10:00',
      score: [1, 0],
    }
    renderNM([final])
    expect(screen.getByText('Spain')).toBeInTheDocument()
    expect(screen.getByText(/World Cup champions!/)).toBeInTheDocument()
    expect(screen.getByText(/Runners-up:.*England/)).toBeInTheDocument()
  })
  ...
  it('falls back to the generic message when the Final is not yet decided', () => {
    ...
    const final = { num: 64, stage: 'Final', t1: 'Spain', t2: 'England', venue: 'stadiumaustralia', ko: '2023-08-20T20:00:00+10:00' }
    renderNM([final])
    expect(screen.getByText(/tournament has concluded/)).toBeInTheDocument()
  ...
  it('falls back to a bullet flag for unknown team names', () => {
    const m = { ...MATCHES[0], t1: 'Nowhereland', t2: 'Atlantis', ko: '2023-12-01T15:00:00+10:00' }
    vi.setSystemTime(new Date('2023-11-01T00:00:00Z'))
    renderNM([m])
