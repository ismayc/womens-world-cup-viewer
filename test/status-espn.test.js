...
function event({ name, state, home = 'Argentina', away = 'Canada', hs, as, description, shortDetail }) {
...
async function recOf(ev, home = 'Argentina', away = 'Canada') {
...
// drives the outcome. Both sides are real Copa teams, so applyLive matches the
// ESPN record by team pair rather than falling back to the kickoff instant.
const bareMatch = (t1 = 'Argentina', t2 = 'Canada') => ({ num: 9001, stage: 'Group', group: 'X', t1, t2, ko: '2024-06-20T20:00:00-04:00' })
...
describe('applyLive — one-off status outcomes on an unscored match', () => {
  it('SUSPENDED -> m.live.delayed && m.live.label === "Suspended"', async () => {
    ...
    const map = new Map([[pairKey('Argentina', 'Canada'), rec]])
