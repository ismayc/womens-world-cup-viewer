...
describe('activeTeams', () => {
  it('collects real teams from unplayed and live matches only', () => {
    const teams = activeTeams([
      m({ stage: 'SF', t1: 'Spain', t2: 'Sweden' }), // unplayed → both active
      m({ stage: 'SF', t1: 'Australia', t2: 'England', score: [1, 0], live: { minute: 70 } }), // live → active
      m({ stage: 'QF', t1: 'Japan', t2: 'Norway', score: [2, 0] }), // final → not active
      m({ stage: 'Final', t1: 'Winner Match 61', t2: 'Winner Match 62' }), // placeholders → skipped
      m({ t1: 'Vietnam', t2: 'Zambia', voided: true }), // voided → skipped
    ])
    expect([...teams].sort()).toEqual(['Australia', 'England', 'Spain', 'Sweden'])
  })
